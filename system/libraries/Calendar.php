...
class Calendar
{
	...
	public function __construct($config = array())
	{
		...
		Fly::log('debug', "Calendar Class Initialized");
	}
	...
	public function generate($year = '', $month = '', $data = array())
	{
		...
		if ($year == '') {
			$year  = date("Y", $this->localTime);
        }

		if ($month == '') {
			$month = date("m", $this->localTime);
        }
		...
		$start_day = ( ! isset($start_days[$this->startDay])) ? 0 : $start_days[$this->startDay];
		...
		if ($this->showNextPrev == TRUE)
		{
			...
			$out .= str_replace('{previous_url}', $this->nextPrevUrl.$adjusted_date['year'].'/'.$adjusted_date['month'], $this->temp['heading_previous_cell']);
			$out .= "\n";
		}
		...
		{
			$adjusted_date = $this->adjustDate($month + 1, $year);
			$out .= str_replace('{next_url}', $this->nextPrevUrl.$adjusted_date['year'].'/'.$adjusted_date['month'], $this->temp['heading_next_cell']);
		}
	 ...
	 * @return	array
	 */
	protected function getDayNames($day_type = '')
	{
		if ($day_type != '')
	...
	{
		$this->temp = $this->getDefaultTemplate();
