...
class Trackback
{
	...
	{
		Fly::log('debug', "Trackback Class Initialized");
	}
	...
	public function send($tb_data)
	{
		...
		foreach (array('url', 'title', 'excerpt', 'blog_name', 'ping_url') as $item) {
			...
			switch ($item) {
				case 'ping_url'	: $$item = $this->extractUrls($tb_data[$item]);
					break;
				case 'excerpt'	: $$item = $this->limitCharacters($this->convertXml(strip_tags(stripslashes($tb_data[$item]))));
					break;
				case 'url'		: $$item = str_replace('&#45;', '-', $this->convertXml(strip_tags(stripslashes($tb_data[$item]))));
					break;
				default			: $$item = $this->convertXml(strip_tags(stripslashes($tb_data[$item])));
					break;
			}
			...
			if ($this->convert_ascii == TRUE) {
				if ($item == 'excerpt') {
					$$item = $this->convertAscii($$item);
				} else if ($item == 'title') {
					$$item = $this->convertAscii($$item);
				} else if ($item == 'blog_name') {
					$$item = $this->convertAscii($$item);
				}
			}
		}
	...
	public function receive()
	{
		foreach (array('url', 'title', 'blog_name', 'excerpt') as $val) {
			...
			$_POST[$val] = ($val != 'url') ? $this->convertXml(strip_tags($_POST[$val])) : strip_tags($_POST[$val]);

			if ($val == 'excerpt') {
				$_POST['excerpt'] = $this->limitCharacters($_POST['excerpt']);
			}
	 ...
	 * @return	void
	 */
	public function sendError($message = 'Incomplete Information')
	{
	 ...
	 * @return	string
	 */
	public function extractUrls($urls)
	{
		...
		array_walk($urls, array($this, 'validateUrl'));
	 ...
	 * @return	string
	 */
	public function convertXml($str)
	{
	 ...
	 * @return	string
	 */
	public function limitCharacters($str, $n = 500, $end_char = '&#8230;')
	{
		if (strlen($str) < $n) {
			return $str;
		}
	 ...
	 * @return	string
	 */
	public function convertAscii($str)
	{
		$count	= 1;
		$out	= '';
		$temp	= array();
