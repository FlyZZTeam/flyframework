...
class Sha1
{
    ...
    {
        Fly::log('debug', "SHA1 Class Initialized");
    }
    ...
    public function generate($str)
    {
        ...
        for ($i = 0; $i < count($x); $i += 16) {
            ...
            for ($j = 0; $j < 80; $j++) {
                ...
                $t = $this->_safeAdd($this->_safeAdd($this->_rol($a, 5), $this->_ft($j, $b, $c, $d)), $this->_safeAdd($this->_safeAdd($e, $w[$j]), $this->_kt($j)));
    ...
    {
        return ($num << $cnt) | $this->_zeroFill($num, 32 - $cnt);
    }
     ...
     * @return string
     */
    private function _zeroFill($a, $b)
    {
        $bin = decbin($a);
