 ...
 * (i.e. Fly_DEBUG is defined as true). Detailed error information with source code
 ...
 * {@link Application::getErrorHandler()}.
 *
...
class ErrorHandler extends ApplicationComponent
{
    ...
    /**
     * @var string the application administrator information (could be a name or email link).
     * It is displayed in error pages to end users. Defaults to 'the webmaster'.
     */
     ...
     * Inside the action, it can retrieve the error information by Fly::app()->errorHandler->error.
    ...
    protected function handleException($exception)
    {
        ...
        if ($app instanceof WebApplication) {
            ...
            if ($exception instanceof HttpException || !FLY_DEBUG) {
    ...
    protected function render($view, $data)
    {
        if ($view === 'error' && $this->errorAction !== null) {
            Fly::app()->runController($this->errorAction);
        } else {
    ...
    {
        $viewPaths = array(
            Fly::app() instanceof WebApplication ? Fly::app()->getSystemViewPath() : null,
            FLY_PATH.DIRECTORY_SEPARATOR.'views',
        );
    ...
    {
        if (FLY_DEBUG) {
            $version = '<a href="http://www.flyframework.com/">FlyFramework</a>/'.Fly::getVersion();
