...
class Trackback
{
    ...
    public function send($tb_data)
    {
        ...
        foreach (array('url', 'title', 'excerpt', 'blog_name', 'ping_url') as $item) {
            ...
            switch ($item) {
                case 'ping_url'    :
                    $$item = $this->extractUrls($tb_data[$item]);
                    break;
                case 'excerpt'    :
                    $$item = $this->limitCharacters($this->convertXml(strip_tags(stripslashes($tb_data[$item]))));
                    break;
                case 'url'        :
                    $$item = str_replace('&#45;', '-', $this->convertXml(strip_tags(stripslashes($tb_data[$item]))));
                    break;
                default            :
                    $$item = $this->convertXml(strip_tags(stripslashes($tb_data[$item])));
                    break;
            }
