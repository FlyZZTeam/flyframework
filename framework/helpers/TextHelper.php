...
class TextHelper
{
    ...
    public static function wordCensor($str, $censored, $replacement = '')
    {
        ...
        foreach ($censored as $badword) {
            if ($replacement != '') {
                ...
                $func = create_function('$m', 'return $m[1].str_repeat("#", strlen($m[2])).$m[3];');
                //"'\\1'.str_repeat('#', strlen('\\2')).'\\3'"

                $str = preg_replace_callback("/({$delim})(".str_replace('\*', '\w*?', preg_quote($badword, '/')).")({$delim})/i",
                    $func,
                    $str);
            }
        }
    ...
    {
        Fly::loadConfig('config.foreign_chars', true);
        $foreignCharacters = Fly::getConfig('foreign_chars');
        if (empty($foreignCharacters)) {
            return $str;
        }
        return preg_replace(array_keys($foreignCharacters), array_values($foreignCharacters), $str);
    }
     ...
     *
     * @param string $str the text string
     * @param integer $charlim the number of characters to wrap at
     * @return string
     */
